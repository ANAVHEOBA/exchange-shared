...
async fn test_create_swap_middleman_address_swap() {
    ...
    println!(
        "Initial estimated amount to user: {}",
    ...
    assert!(
        ...
        "Should show user recipient address"
    );
    ...
    println!(
        "Initial Quote (Net): {}, Final Estimated (Net): {}",
        initial_estimated_amount, final_estimated
    );

    // Allow for up to 1% market volatility between the two API calls
    let diff_percent =
        (final_estimated - initial_estimated_amount).abs() / initial_estimated_amount;
    println!(
        "Market Volatility during test: {:.4}%",
        diff_percent * 100.0
    );

    assert!(
        diff_percent < 0.01,
        "Final estimate should be within 1% of initial net quote (actual diff: {:.4}%)",
        diff_percent * 100.0
    );
    ...
    assert_eq!(
        ...
        user_recipient_address,
        "Should track user recipient address"
    );
}
...
async fn test_middleman_commission_deduction_accuracy() {
    ...
    let network_fee = rate["network_fee"].as_f64().unwrap();
    let platform_fee = rate["platform_fee"].as_f64().unwrap();
    let estimated_amount = rate["estimated_amount"].as_f64().unwrap();

    println!(
        "Network Fee: {}, Platform Fee: {}, Estimated User Receive: {}",
        network_fee, platform_fee, estimated_amount
    );

    // Verify Whale Tier math: platform_fee should be roughly 0.4% of the total
    let total_raw = estimated_amount + platform_fee + network_fee;
    ...
    assert!(
        calculated_fee_percent >= 0.39 && calculated_fee_percent <= 1.51,
        "Fee % ({:.4}%) should be within algorithmic range [0.4%, 1.5%]",
        calculated_fee_percent
    );
}
