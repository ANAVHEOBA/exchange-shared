...
async fn test_get_pairs_multiple_filters() {
    ...
    let response = timed_get(
        &server,
        "/swap/pairs?base_currency=BTC&status=active&size=5",
    )
    .await;
...
async fn test_pairs_response_structure() {
    ...
    if !pairs.is_empty() {
        ...
        assert!(
            pair.get("base_currency").is_some(),
            "Missing 'base_currency' field"
        );
        assert!(
            pair.get("quote_currency").is_some(),
            "Missing 'quote_currency' field"
        );
        assert!(
            pair.get("base_network").is_some(),
            "Missing 'base_network' field"
        );
        assert!(
            pair.get("quote_network").is_some(),
            "Missing 'quote_network' field"
        );
        assert!(pair.get("status").is_some(), "Missing 'status' field");
        assert!(
            pair.get("last_updated").is_some(),
