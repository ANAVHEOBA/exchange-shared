...
async fn test_get_swap_status_successful() {
    ...
    assert!(
        status_json.get("swap_id").is_some(),
        "Response should have swap_id"
    );
    assert!(
        status_json.get("status").is_some(),
        "Response should have status"
    );
    assert!(
        status_json.get("provider").is_some(),
        "Response should have provider"
    );
    assert!(
        status_json.get("provider_swap_id").is_some(),
        "Response should have provider_swap_id"
    );
    assert!(
        status_json.get("from").is_some(),
        "Response should have from"
    );
    assert!(status_json.get("to").is_some(), "Response should have to");
    assert!(
        status_json.get("amount").is_some(),
        "Response should have amount"
    );
    assert!(
        status_json.get("deposit_address").is_some(),
        "Response should have deposit_address"
    );
    assert!(
        status_json.get("recipient_address").is_some(),
        "Response should have recipient_address"
    );
    assert!(
        status_json.get("rate").is_some(),
        "Response should have rate"
    );
    assert!(
        status_json.get("estimated_receive").is_some(),
        "Response should have estimated_receive"
    );
    assert!(
        status_json.get("created_at").is_some(),
