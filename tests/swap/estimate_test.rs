...
async fn test_estimate_large_trade_warning() {
    ...
    let has_large_trade_warning = warnings
        .iter()
        .any(|w| w.as_str().unwrap().contains("Large trade"));
...
async fn test_estimate_vs_rates_consistency() {
    ...
    // Get estimate
    let estimate_url =
        "/swap/estimate?from=btc&to=eth&amount=0.1&network_from=Mainnet&network_to=ERC20";
    ...
    assert!(
        diff_pct < 5.0,
        "Estimate and rates should be consistent (diff: {:.2}%)",
        diff_pct
    );

    println!(
        "✅ Estimate vs Rates: {:.4} vs {:.4} ETH (diff: {:.2}%)",
...
async fn test_estimate_performance() {
    ...
    assert!(
        cache_age >= 0 && cache_age <= 60,
        "Cache age should be 0-60 seconds, got {}",
        cache_age
    );
    ...
    assert!(
        expires_in >= 0,
        "Expires in should be non-negative, got {}",
        expires_in
    );

    println!(
        "✅ Performance: First cached={}, Second cached={}, age={}s, expires_in={}s",
...
async fn test_estimate_multiple_pairs() {
    ...
    for (from, to, amount, network_from, network_to) in pairs {
        ...
        println!(
            "✅ {} -> {}: {} {}",
            from, to, json["estimated_receive"], to
        );
