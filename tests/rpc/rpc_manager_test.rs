use exchange_shared::services::rpc::{
    config::{CircuitBreakerConfig, LoadBalancingStrategy, RpcAuth, RpcConfig, RpcEndpoint},
    manager::RpcManager,
};
...
async fn test_weighted_round_robin_strategy() {
    ...
    let high_count = *selections
        .get("https://high-weight.example.com")
        .unwrap_or(&0);
    let low_count = *selections
        .get("https://low-weight.example.com")
        .unwrap_or(&0);

    // High weight endpoint should be selected ~3x more often
    assert!(
        high_count > low_count,
        "High weight endpoint should be selected more: high={}, low={}",
        high_count,
        low_count
    );
    ...
    assert!(
        ratio > 2.0 && ratio < 4.0,
        "Weight ratio should be around 3:1, got {:.2}",
        ratio
    );
}
...
async fn test_health_score_based_strategy() {
    ...
    for _ in 0..5 {
        manager
            .record_result(&endpoint1, Duration::from_millis(100), false, None)
            .await;
    }
    ...
    for _ in 0..5 {
        manager
            .record_result(endpoint2, Duration::from_millis(50), true, Some(1000))
            .await;
    }
...
async fn test_least_latency_strategy() {
    ...
    let selected = manager.select_endpoint("test").await.unwrap();
    assert_eq!(
        selected, "https://fast.example.com",
...
async fn test_priority_ordering() {
    ...
    let endpoint = manager.select_endpoint("test").await.unwrap();
    assert_eq!(
        endpoint, "https://high-priority.example.com",
...
async fn test_circuit_breaker_opens_on_failures() {
    ...
    let selected = manager.select_endpoint("test").await.unwrap();
    assert_eq!(
        selected, "https://backup.example.com",
        "Should failover to backup endpoint"
    );

    let health_status = manager.get_health_status("test").await;
    let failing_status = health_status
        .iter()
        .find(|s| s.url == "https://failing.example.com")
        .unwrap();
    assert_eq!(
        failing_status.state, "Open",
...
async fn test_no_healthy_endpoints_error() {
    ...
    assert!(
        result.is_err(),
        "Should fail when no healthy endpoints available"
    );
}
...
async fn test_latency_tracking() {
    ...
    assert!(
        avg_latency > 140.0 && avg_latency < 160.0,
        "Average latency should be around 150ms, got {}",
        avg_latency
    );
}
...
async fn test_p95_latency_calculation() {
    ...
    assert!(
        p95 >= 100 && p95 <= 1000,
        "P95 latency should be in range, got {}",
        p95
    );
}
...
async fn test_block_height_tracking() {
    ...
    let health_status = manager.get_health_status("test").await;
    assert_eq!(
        health_status[0].last_block_height,
        Some(1009),
        "Should track latest block height"
    );
}
...
async fn test_health_status_for_nonexistent_chain() {
    ...
    assert_eq!(
        health_status.len(),
        0,
        "Should return empty vec for non-existent chain"
    );
}
...
async fn test_concurrent_endpoint_selection() {
    ...
    for _ in 0..10 {
        ...
        let handle = tokio::spawn(async move { manager_clone.select_endpoint("test").await });
        handles.push(handle);
    }
