...
async fn test_resilience_to_external_api_timeout() {
    ...
    assert!(
        result.is_ok(),
        "Engine should gracefully handle API failures"
    );
    ...
    assert_ne!(
        swap_status.status, "completed",
        "Swap should not be completed when API fails"
    );
...
fn test_math_bounds_extreme_elapsed_time() {
    ...
    assert!(
        interval_year <= Duration::from_secs(3600),
        "Should be capped at safety max"
    );
}
...
fn test_probability_distribution_integrity() {
    ...
    assert!(
        p2 > p1,
        "Likelihood of completion must increase as we approach the median completion time"
    );
}
