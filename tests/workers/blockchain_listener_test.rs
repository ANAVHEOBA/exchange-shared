...
async fn test_listener_detects_funds() {
    ...
    create_swap_waiting_for_funds(&ctx.db, &swap_id, our_address, "ethereum", 1.0, 0.012).await;
    ...
    assert!(
        ...
        "Expected at least 1 pending swap, got {}",
        stats.total_pending
    );

    println!(
        "✅ Listener initialized with {} pending swaps",
        stats.total_pending
    );
...
async fn test_listener_handles_multiple_chains() {
    ...
    assert!(
        stats.total_pending >= 2,
        "Expected at least 2 pending swaps, got {}",
        stats.total_pending
    );
    println!(
        "✅ Listener tracking {} swaps across multiple chains",
        stats.total_pending
    );
...
async fn test_listener_ignores_old_swaps() {
    ...
    println!(
        "Old swaps in DB: {}, Listener pending: {}",
        old_count, stats.total_pending
    );
