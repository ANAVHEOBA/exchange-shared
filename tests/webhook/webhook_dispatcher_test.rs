use chrono::Utc;
use exchange_shared::services::webhook::{RetryConfig, Webhook, WebhookDispatcher, WebhookPayload};
...
async fn test_idempotency_check() {
    ...
    let count: i64 =
        sqlx::query_scalar("SELECT COUNT(*) FROM webhook_deliveries WHERE swap_id = ?")
...
async fn test_multiple_webhooks_per_swap() {
    ...
    let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM webhooks WHERE swap_id = ?")
        .bind(swap_id.to_string())
        .fetch_one(&pool)
        .await
        .unwrap();
