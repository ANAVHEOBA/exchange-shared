use exchange_shared::services::rpc::{build_default_rpc_configs, RpcManager, RpcManagerAdapter};
...
fn test_rpc_config_builder() {
    ...
    assert!(
        configs.len() >= 100,
        "Should have at least 100 chain configs, got {}",
        configs.len()
    );

    // Check key chains exist
    assert!(
        configs.contains_key("ethereum"),
        "Should have Ethereum config"
    );
    assert!(
        configs.contains_key("bitcoin"),
        "Should have Bitcoin config"
    );
    assert!(configs.contains_key("solana"), "Should have Solana config");
    assert!(
        configs.contains_key("bnb_smart_chain"),
        "Should have BSC config"
    );
    assert!(
        configs.contains_key("polygon"),
        "Should have Polygon config"
    );
    assert!(
        configs.contains_key("arbitrum_one"),
        "Should have Arbitrum config"
    );
    assert!(configs.contains_key("base"), "Should have Base config");
    assert!(
        configs.contains_key("avalanche_c_chain"),
        "Should have Avalanche config"
    );
    assert!(
        configs.contains_key("optimism"),
        "Should have Optimism config"
    );
    ...
    println!(
        "✅ RPC config builder creates {} chain configs",
        configs.len()
    );
    ...
    println!(
        "Sample chains: {:?}",
        &chain_names[..10.min(chain_names.len())]
    );
}
