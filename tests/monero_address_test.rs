...
async fn test_monero_address_generation() {
    ...
    let mnemonic = std::env::var("WALLET_MNEMONIC").expect("WALLET_MNEMONIC must be set");
    ...
    assert!(
        ...
        "Monero mainnet address should start with '4'"
    );
    ...
    let is_valid = client
        .validate_address("xmr", "Mainnet", &address)
        .await
    ...
    assert!(
        is_valid,
        "Generated Monero address should be valid according to Trocador"
    );
}
