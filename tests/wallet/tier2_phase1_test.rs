...
async fn test_litecoin_dispatcher_aliases() {
    ...
    let addr_litecoin = derive_address(
...
async fn test_dogecoin_dispatcher_aliases() {
    ...
    let addr_dogecoin = derive_address(
        &crate::common::test_wallet_mnemonic(),
        "dogecoin",
        "dogecoin",
        2,
...
async fn test_bitcoin_cash_uniqueness_per_index() {
    ...
    for i in 0..10 {
        ...
        .await
        .expect(&format!(
            "Failed to derive Bitcoin Cash address at index {}",
            i
        ));
...
async fn test_bitcoin_cash_dispatcher_aliases() {
    ...
    let addr_bitcoincash = derive_address(
        &crate::common::test_wallet_mnemonic(),
        "bitcoin_cash",
        "bitcoin_cash",
        1,
...
async fn test_tier2_addresses_different_from_each_other() {
    ...
    assert_ne!(ltc, doge, "Litecoin and Dogecoin addresses must differ");
    assert_ne!(ltc, bch, "Litecoin and Bitcoin Cash addresses must differ");
    assert_ne!(doge, bch, "Dogecoin and Bitcoin Cash addresses must differ");
}
