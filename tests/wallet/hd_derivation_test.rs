...
use exchange_shared::services::wallet::{
    derive_btc_address, derive_evm_address, derive_evm_key, derive_solana_address,
    derive_sui_address, is_valid_seed_phrase, sign_message_with_seed,
};
...
async fn test_different_derivation_paths() {
    ...
    assert_ne!(
        address_0, address_1,
        "Index 0 and 1 should produce different addresses"
    );
    assert_ne!(
        address_1, address_2,
        "Index 1 and 2 should produce different addresses"
    );
    assert_ne!(
        address_0, address_2,
        "Index 0 and 2 should produce different addresses"
    );

    assert!(
        duration.as_millis() < 1000,
        "Batch derivation too slow: {:?}",
        duration
    );
...
async fn test_evm_chain_derivation_path() {
    ...
    assert!(
        duration.as_millis() < 500,
        "EVM derivation took too long: {:?}",
        duration
    );

    println!(
        "EVM Address (Ethereum/Polygon/Arbitrum): {} (took {:?})",
        eth_address, duration
    );
}
...
async fn test_bitcoin_derivation_path() {
    ...
    assert!(
        btc_address.starts_with("bc1")
            || btc_address.starts_with("1")
            || btc_address.starts_with("3"),
        "Bitcoin address should be valid format"
    );

    assert!(
        duration.as_millis() < 500,
        "BTC derivation took too long: {:?}",
        duration
    );
...
async fn test_solana_derivation_path() {
    ...
    assert!(
        !sol_address.is_empty(),
        "Solana address should not be empty"
    );
    assert!(
        duration.as_millis() < 500,
        "Solana derivation took too long: {:?}",
        duration
    );
...
async fn test_sui_derivation_path() {
    ...
    assert!(
        sui_address.starts_with("0x"),
        "Sui address should start with 0x"
    );
    assert!(
        duration.as_millis() < 500,
        "Sui derivation took too long: {:?}",
        duration
    );
...
async fn test_high_address_index_derivation() {
    ...
    assert!(
        duration.as_secs() < 10,
        "Batch of 100 derivations too slow: {:?}",
        duration
    );

    println!(
        "✅ Generated 100 unique addresses from same seed (took {:?})",
        duration
    );
}
...
async fn test_signing_consistency() {
    ...
    let signature_1 = sign_message_with_seed(&seed_phrase, 0, message)
        .await
        .unwrap();
    let signature_2 = sign_message_with_seed(&seed_phrase, 0, message)
        .await
        .unwrap();
