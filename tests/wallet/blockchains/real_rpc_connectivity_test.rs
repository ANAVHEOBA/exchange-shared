...
use exchange_shared::config::rpc_config::{load_rpc_config, get_rpc_config};
...
async fn test_all_133_blockchains_real_rpc_connectivity() {
    ...
    println!("📈 Success Rate: {:.1}%", (connected as f64 / total as f64) * 100.0);
    ...
    println!("   Alchemy: {}", provider_stats.get("Alchemy").unwrap_or(&0));
...
async fn test_evm_chains_gas_price_fetch() {
    ...
    let config = load_rpc_config();
    let evm_chains: Vec<_> = config.iter()
        .filter(|(_, endpoint)| matches!(endpoint.protocol, exchange_shared::config::rpc_config::BlockchainProtocol::EVM))
        .collect();
    ...
    println!("❌ Failed: {}", failed);
    println!("📈 Success Rate: {:.1}%", (success as f64 / evm_chains.len() as f64) * 100.0);
}
...
async fn test_fallback_rpc_endpoints() {
    ...
    for chain_name in chains_with_fallbacks {
        if let Some(config) = get_rpc_config(chain_name) {
            println!("\n{} ({})", chain_name.to_uppercase(), config.fallbacks.len());
            println!("  Primary: {}", &config.primary[..60.min(config.primary.len())]);
            
            for (i, fallback) in config.fallbacks.iter().enumerate() {
                let client = HttpRpcClient::new(fallback.clone());
                match client.get_gas_price().await {
                    Ok(_) => println!("  ✅ Fallback #{}: {}", i + 1, &fallback[..60.min(fallback.len())]),
                    Err(_) => println!("  ❌ Fallback #{}: {}", i + 1, &fallback[..60.min(fallback.len())]),
                }
            }
        }
    }
