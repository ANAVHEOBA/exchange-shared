...
mod priority_tier_1_blockchains {
    ...
    async fn test_cardano_address_generation() {
        ...
        let addr =
            exchange_shared::services::wallet::derivation::derive_cardano_address(&mnemonic, 0)
                .await
    ...
    async fn test_polkadot_address_generation() {
        let mnemonic = crate::common::test_wallet_mnemonic();
        let addr =
            exchange_shared::services::wallet::derivation::derive_polkadot_address(&mnemonic, 0)
                .await
    ...
    async fn test_ripple_address_generation() {
        let mnemonic = crate::common::test_wallet_mnemonic();
        let addr =
            exchange_shared::services::wallet::derivation::derive_ripple_address(&mnemonic, 0)
                .await
        ...
        for c in addr.chars() {
            assert!(
                !"0OIl".contains(c),
                "Ripple address contains invalid Base58 character"
            );
        }
        println!("✓ Ripple: {}", addr);
    }
    ...
    async fn test_cosmos_address_generation() {
        let mnemonic = crate::common::test_wallet_mnemonic();
        let addr =
            exchange_shared::services::wallet::derivation::derive_cosmos_address(&mnemonic, 0)
                .await
                .expect("Cosmos derivation failed");

        assert!(
            addr.starts_with("cosmos1"),
            "Cosmos must start with cosmos1"
        );
        println!("✓ Cosmos: {}", addr);
    }
    ...
    async fn test_deterministic_same_index() {
        ...
        let mnemonic = crate::common::test_wallet_mnemonic();
        let cardano_1 =
            exchange_shared::services::wallet::derivation::derive_cardano_address(&mnemonic, 42)
                .await
                .unwrap();
        let cardano_2 =
            exchange_shared::services::wallet::derivation::derive_cardano_address(&mnemonic, 42)
        ...
        let polkadot_1 =
            exchange_shared::services::wallet::derivation::derive_polkadot_address(&mnemonic, 42)
                .await
                .unwrap();
        let polkadot_2 =
            exchange_shared::services::wallet::derivation::derive_polkadot_address(&mnemonic, 42)
    ...
    async fn test_unique_per_index() {
        ...
        for i in 0..10 {
            cardano_set.insert(
                exchange_shared::services::wallet::derivation::derive_cardano_address(&mnemonic, i)
                    .await
                    .unwrap(),
            );
            polkadot_set.insert(
                exchange_shared::services::wallet::derivation::derive_polkadot_address(
                    &mnemonic, i,
                )
                .await
                .unwrap(),
            ...
            ripple_set.insert(
                exchange_shared::services::wallet::derivation::derive_ripple_address(&mnemonic, i)
                    .await
                    .unwrap(),
            );
            tron_set.insert(
                exchange_shared::services::wallet::derivation::derive_tron_address(&mnemonic, i)
                    .await
                    .unwrap(),
            );
            cosmos_set.insert(
                exchange_shared::services::wallet::derivation::derive_cosmos_address(&mnemonic, i)
                    .await
                    .unwrap(),
            );
        }
    ...
    async fn test_dispatcher_aliases() {
        ...
        let mnemonic = crate::common::test_wallet_mnemonic();
        let c1 = exchange_shared::services::wallet::derivation::derive_address(
            &mnemonic, "ADA", "cardano", 0,
        )
        .await
        .unwrap();
        let c2 = exchange_shared::services::wallet::derivation::derive_address(
            &mnemonic, "ADA", "ada", 0,
        )
        .await
        .unwrap();
        ...
        let p1 = exchange_shared::services::wallet::derivation::derive_address(
            &mnemonic, "DOT", "polkadot", 0,
        )
        .await
        .unwrap();
        let p2 = exchange_shared::services::wallet::derivation::derive_address(
            &mnemonic, "DOT", "dot", 0,
        )
        .await
        .unwrap();
