...
use super::config::{LoadBalancingStrategy, RpcAuth, RpcConfig, RpcEndpoint};
...
pub struct RpcHealthOverview {
    pub configured_chains: usize,
    pub total_endpoints: usize,
    pub sampled_endpoints: usize,
...
impl RpcManager {
    ...
    pub async fn select_endpoint(&self, chain: &str) -> Result<String, RpcError> {
        ...
        let mut candidates: Vec<&RpcEndpoint> = config
            .endpoints
            .iter()
            .filter(|ep| {
                health
    ...
    pub async fn call<T: DeserializeOwned>(
        ...
        for attempt in 0..max_attempts {
            ...
            let result = self
                .execute_rpc_call(&url, method, params.clone(), endpoint)
                .await;
        ...
        loop {
            for (chain, config) in &self.configs {
                for endpoint in &config.endpoints {
                    ...
                    self.record_result(&endpoint.url, latency, success, block_height)
                        .await;
                }
            }
    ...
    pub async fn get_health_status(&self, chain: &str) -> Vec<EndpointHealthStatus> {
        ...
        config
            .endpoints
            .iter()
            .filter_map(|ep| health.get(&ep.url).map(|h| EndpointHealthStatus::from(h)))
            .collect()
    }

    pub async fn health_overview(&self) -> RpcHealthOverview {
        let health = self.health_tracker.read().await;
        let sampled_endpoints = health.values().filter(|h| h.total_requests > 0).count();
        let healthy_endpoints = health
            .values()
            .filter(|h| h.total_requests > 0 && h.is_healthy())
            .count();

        RpcHealthOverview {
            configured_chains: self.configs.len(),
