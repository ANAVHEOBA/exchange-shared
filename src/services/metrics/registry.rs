...
impl MetricsRegistry {
    pub fn new() -> Result<Arc<Self>, Box<dyn std::error::Error>> {
        ...
        let http_requests_total = CounterVec::new(
            Opts::new("exchange_http_requests_total", "Total HTTP requests").namespace("exchange"),
        ...
        let swap_failed_total = CounterVec::new(
            Opts::new("exchange_swap_failed_total", "Total swaps failed").namespace("exchange"),
        ...
        let swap_processing_duration_seconds = HistogramVec::new(
            HistogramOpts::new(
                "exchange_swap_processing_duration_seconds",
                "Swap processing duration",
            )
            .namespace("exchange")
            .buckets(vec![
        ...
        let swap_active_count = GaugeVec::new(
            Opts::new("exchange_swap_active_count", "Active swaps by status").namespace("exchange"),
            &["status"],
        )?;
        ...
        let payout_failed_total = CounterVec::new(
            Opts::new("exchange_payout_failed_total", "Total payouts failed").namespace("exchange"),
        ...
        let payout_duration_seconds = HistogramVec::new(
            HistogramOpts::new(
                "exchange_payout_duration_seconds",
        ...
        let payout_gas_cost = HistogramVec::new(
            HistogramOpts::new(
                "exchange_payout_gas_cost",
                "Payout gas cost in native currency",
            )
            .namespace("exchange")
        ...
        let rpc_endpoint_health_score = GaugeVec::new(
            Opts::new(
                "exchange_rpc_endpoint_health_score",
                "RPC endpoint health score (0.0-1.0)",
            )
            .namespace("exchange"),
            &["chain", "endpoint"],
        )?;
        ...
        let rpc_requests_total = CounterVec::new(
            Opts::new("exchange_rpc_requests_total", "Total RPC requests").namespace("exchange"),
        ...
        let rpc_request_duration_seconds = HistogramVec::new(
            HistogramOpts::new(
                "exchange_rpc_request_duration_seconds",
        ...
        let rpc_circuit_breaker_state = GaugeVec::new(
            Opts::new(
                "exchange_rpc_circuit_breaker_state",
        ...
        let rpc_block_height_lag = GaugeVec::new(
            Opts::new(
                "exchange_rpc_block_height_lag",
                "Block height lag from highest known",
            )
            .namespace("exchange"),
            &["chain", "endpoint"],
        )?;
        ...
        let cache_hit_ratio = GaugeVec::new(
            Opts::new("exchange_cache_hit_ratio", "Cache hit ratio").namespace("exchange"),
            &["cache", "key_pattern"],
        )?;
        registry.register(Box::new(cache_hit_ratio.clone()))?;

        let cache_size_bytes = GaugeVec::new(
            Opts::new("exchange_cache_size_bytes", "Cache size in bytes").namespace("exchange"),
            &["cache"],
        )?;
        registry.register(Box::new(cache_size_bytes.clone()))?;

        let cache_entries_total = GaugeVec::new(
            Opts::new("exchange_cache_entries_total", "Total cache entries").namespace("exchange"),
            &["cache"],
        )?;
        ...
        let cache_operation_duration_seconds = HistogramVec::new(
            HistogramOpts::new(
                "exchange_cache_operation_duration_seconds",
        ...
        let db_queries_total = CounterVec::new(
            Opts::new("exchange_db_queries_total", "Total database queries").namespace("exchange"),
        ...
        let db_query_duration_seconds = HistogramVec::new(
            HistogramOpts::new(
                "exchange_db_query_duration_seconds",
        ...
        let db_connections_idle =
            Gauge::new("exchange_db_connections_idle", "Idle database connections")?;
        ...
        let revenue_total_usd = CounterVec::new(
            Opts::new("exchange_revenue_total_usd", "Total revenue in USD").namespace("exchange"),
            &["currency", "fee_type"],
        )?;
        registry.register(Box::new(revenue_total_usd.clone()))?;

        let tvl_usd = GaugeVec::new(
            Opts::new("exchange_tvl_usd", "Total Value Locked in USD").namespace("exchange"),
            &["currency"],
        )?;
        registry.register(Box::new(tvl_usd.clone()))?;

        let user_swaps_total = CounterVec::new(
            Opts::new("exchange_user_swaps_total", "Total user swaps").namespace("exchange"),
            &["user_tier"],
        )?;
        ...
        let commission_per_swap_usd = HistogramVec::new(
            HistogramOpts::new(
                "exchange_commission_per_swap_usd",
                "Commission per swap in USD",
            )
            .namespace("exchange")
