...
impl TronTransaction {
    ...
    pub fn sign(&mut self, private_key_hex: &str) -> Result<(), String> {
        ...
        // Hash transaction ID
        let tx_id_bytes = hex::decode(&self.tx_id).map_err(|e| format!("Invalid tx_id: {}", e))?;
        ...
        // Add signature
        self.signature
            .push(hex::encode(signature.serialize_compact()));
...
pub async fn get_tron_block_info(rpc_url: &str) -> Result<TronBlockInfo, String> {
    ...
    let block_id = block["blockID"].as_str().ok_or("Missing blockID")?;
