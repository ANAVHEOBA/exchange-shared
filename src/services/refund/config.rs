...
impl RefundConfig {
    ...
    pub fn from_env() -> Result<Self, String> {
        ...
        if let Ok(val) = std::env::var("REFUND_DEPOSIT_TIMEOUT") {
            config.deposit_timeout = val
                .parse()
                .map_err(|e| format!("Invalid REFUND_DEPOSIT_TIMEOUT: {}", e))?;
        }

        if let Ok(val) = std::env::var("REFUND_MAX_RETRY_ATTEMPTS") {
            config.max_retry_attempts = val
                .parse()
                .map_err(|e| format!("Invalid REFUND_MAX_RETRY_ATTEMPTS: {}", e))?;
        }

        if let Ok(val) = std::env::var("REFUND_CHECK_INTERVAL") {
            config.check_interval = val
                .parse()
                .map_err(|e| format!("Invalid REFUND_CHECK_INTERVAL: {}", e))?;
        }

        // Validate weights sum to 1.0
        let weight_sum = config.priority_weight_age
            + config.priority_weight_amount
            + config.priority_weight_retry;
        if (weight_sum - 1.0).abs() > 0.01 {
            return Err(format!(
                "Priority weights must sum to 1.0, got {}",
                weight_sum
            ));
        }
